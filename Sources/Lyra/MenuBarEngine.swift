...
public final class MenuBarEngine: NSObject {
    ...
    public func update(state: AppPlaybackState, lyrics: [LyricLine], status: LyricsStatus) {
        let title: String
        ...
        if !areLyricsEnabled {
            title = ""
            showOnlyIcon = true
        } else if !state.isSpotifyRunning {
            title = ""
            showOnlyIcon = true
            ...
            let currentPos = getCurrentPosition(state: state)
